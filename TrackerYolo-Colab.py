...
class Tracker:
    def __init__(self, vp, nl, file_id):
        ...
        print('Number of trials current session', self.num_trials, '\nGoal location node ', self.goal)   
    ...
    def find_start(self, center_rat):
        ...
        if 2 < points_dist(center_rat, node) < 55: 
                       self.trial_num += 1
          ...
          for i in indexes.flatten():
            ...
            if label == 'rat':
                ...
                if center_rat is not None:
                    self.count_rat += 1                   
                    if self.start == True:
                         # print ('\nDetection rat time', self.converted_time, '\nposition ', center_rat, self.start_nodes_locations[self.trial_num])     
    ...
    def annotate_node(frame, point, node, t):
        ...
        if t==1:
           ...
           cv2.putText(frame, str(node), (point[0]- 13, point[1]), 
        ...
        if t==2:
           ...
           cv2.putText(frame, str(node), (point[0] -13, point[1]), 
                    ...
                    lineType=cv2.LINE_AA) 
        if t==3:
           cv2.circle(frame, point, 20, color = (0, 0, 250), thickness = 1)
           cv2.putText(frame, str(node), (point[0] -13, point[1]), 
                    fontScale=0.5, fontFace=FONT, color = (0, 0, 250), thickness=1,
                    lineType=cv2.LINE_AA)
           cv2.putText(frame, 'End', (point[0]- 17, point[1]-22), 
                    fontScale=0.5, fontFace=FONT, color = (0, 0, 250), thickness=1,
                    lineType=cv2.LINE_AA)
    ...
    def annotate_frame(self, frame):
            ...
            for node_name in nodes_dict:
                if points_dist(self.pos_centroid, nodes_dict[node_name]) < 20:                    
                    if self.record_detections: #condition to go into 'save mode'
                        ...
                        print('\nTrial', self.trial_num, 'Time', self.converted_time, '\nNode ', node_name,'\nFPS ', fps)
        ...
        if self.record_detections:          
            ...
            cv2.putText(frame,'Currently writing to file...', (60,80), 
                        fontFace = FONT, fontScale = 0.75, color = (255,255,255), thickness = 1)           
            cv2.putText(frame, "Rat Count: " + str(self.count_rat), (30,140), fontFace = FONT, fontScale = 0.65, color = (255,255,255), thickness = 1)
            cv2.putText(frame, "Rat head Count: " + str(self.count_head), (30,160), fontFace = FONT, fontScale = 0.65, color = (255,255,255), thickness = 1)
          ...
          #  if len(self.path)>3:    #[rat position trace] 
           #     for j in range(1, len(self.path)):
            #      if self.path[j-1] is None or self.path[j] is None:
             #        continue
            #thickness = int(np.sqrt(64/float(j+1))*2)  ##j-1
               #   cv2.line(frame, (self.path[j-1]), (self.path[j]),  color = (0,255, 255), thickness = 2)     
          #  if self.pos_centroid is not None:
        ...
        for node in start_nodes:
          for node_name in nodes_dict: 
               if node_name == str(node):  
                ...
                # print('\nLocation found',nodes_dict[node_name],'start node', node, node_name)            
...
if __name__ == "__main__":
 ...
 #   parser.add_argument('-o', "--output", dest='output', help='full path for saving processed video output')
    ...
    #vid_path = 'Rat12_1_20200605_stitched.mp4' 
    print('\nVideo path' , args.vid_path)
   # vid_path = args
 ...
 #   vid_path = gui.vpath
   ...
   # logger.info('File saved in: {}'.format(save_path))
    ...
    Tracker(vp = args.vid_path, nl = node_list, file_id = file_id)
